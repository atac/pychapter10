...
if __name__=='__main__':
    ...
    numpkts = EasyDialogs.AskString("How many packets to dump? ")
    
    for i in range(int(numpkts)):
        pkt_header = INfp.read(ch10_pkt_header_length)
        obj = Chapter10File(pkt_header)
        idum = INfp.read(obj.pkt_len-24)
        print "Channel = %d Type = %d" % (obj.chan_ID,obj.data_typ)
